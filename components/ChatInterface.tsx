...
// --- ФУНКЦИЯ-ПОМОЩНИК: Превращает временную ссылку в вечный Base64 ---
async function convertLinkToBase64(url: string): Promise<string> {
  try {
    ...
    console.error("Ошибка конвертации в браузер:", e);
    return url; // В крайнем случае вернем исходную ссылку
  }
}
...
export default function ChatInterface() {
  ...
  const handleSendImage = async (e: React.FormEvent) => {
    ...
    try {
      // 1. Пишем запрос в чат
      await supabase.from('messages').insert([{ content: `✨ Vision (FLUX): ${prompt}`, is_mine: true }] as any); 

      // 2. Просим сервер дать нам ссылку
      ...
      if (!response.ok) throw new Error('Ошибка сервера генерации');
      const data = await response.json();
      
      if (data.imageUrl) {
        console.log("Ссылка получена, браузер начинает скачивание...");
        // 3. БРАУЗЕР скачивает картинку и делает её вечной
        const permanentBase64 = await convertLinkToBase64(data.imageUrl);

        // 4. Сохраняем вечный код в базу
        await supabase.from('messages').insert([{
          content: '', 
          is_mine: false,
          image_url: permanentBase64 // Сохраняем Base64!
        }] as any);
      }
  ...
  return (
    <div className="flex flex-col h-[100dvh] w-full max-w-md mx-auto bg-black text-[#D4AF37] font-sans overflow-hidden shadow-2xl">
      <div className="p-4 border-b border-[#1F1F1F] bg-black text-center shrink-0">
        ...
        <p className="text-[10px] text-zinc-500 uppercase tracking-widest mt-1">Secure Uplink (FLUX)</p>
      </div>
              ...
              {msg.image_url && (
                <div className="w-full mb-2 overflow-hidden rounded-lg border border-white/5">
                  <img 
                    src={msg.image_url} 
                    alt="Vision" 
                    className="w-full h-auto block object-contain min-h-[100px] bg-zinc-900"
                    loading="eager"
                  />
                </div>
              )}
      ...
      <div className="p-4 bg-black border-t border-[#1F1F1F] flex flex-col gap-3 pb-safe">
        <form onSubmit={handleSendText} className="flex gap-2">
          <input type="text" value={newMessage} onChange={(e) => setNewMessage(e.target.value)} placeholder="Сообщение..." className="flex-1 bg-[#121212] border border-zinc-800 text-[#D4AF37] p-3 rounded-xl focus:outline-none focus:border-[#D4AF37]/50 text-sm" />
        ...
        <form onSubmit={handleSendImage} className="flex gap-2">
          <input type="text" value={newImagePrompt} onChange={(e) => setNewImagePrompt(e.target.value)} placeholder="Опишите видение (FLUX)..." className="flex-1 bg-[#121212] border border-zinc-800 text-[#D4AF37] p-3 rounded-xl focus:outline-none focus:border-[#D4AF37]/50 text-sm" />
